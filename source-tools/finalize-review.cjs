...
if (!read('engine.js').includes('    quickEndgame: input.quickEndgame !== false,')) {
  ...
  edit('engine.js', '  // Every eligible faction retains its turn, including late-game defenders.\n  for (const factionId of actionOrder) {', `  // Fast completion is an explicit legacy mode, not competitive simulation.
...
console.log('Finalized legal-front modes, import validation and cache revision.');
